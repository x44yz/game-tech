...
public class Missile : MonoBehaviour
{
    ...
    void DoCollision(Collision collision, Collider other)
    {
        ...
        if (collision != null && other == null)
            entityBehaviour = collision.gameObject.transform.GetComponent<Actor>();
        else if (collision == null && other != null)
            entityBehaviour = other.gameObject.transform.GetComponent<Actor>();
        else
            return;
    ...
    public static Actor GetEntityTargetInTouchRange(Vector3 aimPosition, Vector3 aimDirection)
    {
        ...
        if (Physics.SphereCast(ray, SphereCastRadius, out hit, TouchRange))
            return hit.transform.GetComponent<Actor>();
        else
            return null;
    }
    ...
    void DoTouch()
    {
        ...
        Actor entityBehaviour = GetEntityTargetInTouchRange(GetAimPosition(), GetAimDirection());
    ...
    void DoAreaOfEffect(Vector3 position, bool ignoreCaster = false)
    {
        ...
        {
            Actor aoeEntity = overlaps[i].GetComponent<Actor>();
