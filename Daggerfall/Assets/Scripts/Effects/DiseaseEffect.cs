...
public abstract class DiseaseEffect : IncumbentEffect
{
    ...
    public override void Start(ActorEffect manager, Actor caster = null)
    {
        ...
        Actor host = GetPeeredEntityBehaviour(manager);
        if (host.EntityType != EntityTypes.Player || host.Level < 2)
        {
            EndDisease();
            return;
        }
    ...
    protected virtual void IncrementDailyDiseaseEffects()
    {
        ...
        host.DecreaseHealth(diseaseData.HEA * damageAmount);
        host.DecreaseFatigue(diseaseData.FAT * damageAmount);
        host.DecreaseMagicka(diseaseData.SPL * damageAmount);
    }
