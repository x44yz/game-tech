...
public class PoisonEffect : IncumbentEffect
{
    ...
    public override void Start(ActorEffect manager, Actor caster = null)
    {
        ...
        var host = GetPeeredEntityBehaviour(manager);
        // Debug.Log(host.Name + " afflicted with " + PoisonType + ", starting in " + minutesToStart + " minutes, lasting for " + minutesRemaining + " minutes.");
    }
    ...
    protected virtual void IncrementPoisonEffects()
    {
        ...
        switch (PoisonType)
        {
            case Poisons.Nux_Vomica:
                host.DecreaseHealth(Random.Range(2, 12));
                break;

            case Poisons.Arsenic:
                host.DecreaseHealth(2);
                ChangeStatMod(DFCareer.Stats.Endurance, -1);
                break;

            case Poisons.Moonseed:
                host.DecreaseHealth(Random.Range(1, 10));
                break;
            ...
            case Poisons.Somnalius:
                host.DecreaseFatigue(Random.Range(10, 100), true);
                break;

            case Poisons.Pyrrhic_Acid:
                host.DecreaseHealth(Random.Range(1, 30));
                break;

            case Poisons.Magebane:
                ChangeStatMod(DFCareer.Stats.Willpower, -Random.Range(1, 5));
                host.DecreaseMagicka(Random.Range(5, 15));
                break;
            ...
            case Poisons.Quaesto_Vil:
                ...
                host.IncreaseFatigue(Random.Range(5, 10), true);
                break;

            case Poisons.Aegrotat:
                ChangeStatMod(DFCareer.Stats.Endurance, -Random.Range(1, 5));
                host.IncreaseMagicka(Random.Range(5, 10));
                break;
        }
