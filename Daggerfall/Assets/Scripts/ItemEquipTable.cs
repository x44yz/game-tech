...
public class ItemEquipTable
{
    ...
    readonly Actor parentEntity = null;
    Item[] equipTable = new Item[equipTableLength];
    ...
    public ItemEquipTable(Actor parentEntity)
    {
    ...
    public void Clear()
    {
        equipTable = new Item[equipTableLength];
    }
    ...
    public List<Item> EquipItem(Item item, bool alwaysEquip = true, bool playEquipSounds = true)
    {
        if (item == null)
            return null;
        ...
        if (item.IsAStack())
            item = Main.Inst.hero.Items.SplitStack(item, 1);
        
        List<Item> unequippedList = new List<Item>();
        ...
        if (GetItemHands(item) == ItemHands.LeftOnly)
        {
            ...
            Item rightHandItem = equipTable[(int)EquipSlots.RightHand];
    ...
    public void UnequipItem(EquipSlots slot, List<Item> list)
    {
        Item item = UnequipItem(slot);
        if (item != null)
            list.Add(item);
    ...
    public Item UnequipItem(EquipSlots slot)
    {
        if (!IsSlotOpen(slot))
        {
            Item item = equipTable[(int)slot];
    ...
    public EquipSlots GetEquipSlot(Item item)
    {
    ...
    EquipSlots GetJewellerySlot(Item item)
    {
    ...
    EquipSlots GetArmorSlot(Item item)
    {
        ItemTemplate template = item.ItemTemplate;
    ...
    EquipSlots GetWeaponSlot(Item item)
    {
    ...
    EquipSlots GetMensClothingSlot(Item item)
    {
        ItemTemplate template = item.ItemTemplate;
    ...
    EquipSlots GetWomensClothingSlot(Item item)
    {
        ItemTemplate template = item.ItemTemplate;
    ...
    void StartEquippedItem(Item item)
    {
    ...
    void StopEquippedItem(Item item)
    {
        if (parentEntity != null && parentEntity)
        {
            EffectManager manager = parentEntity.GetComponent<EffectManager>();
    ...
    /// </summary>
    public static ItemHands GetItemHands(Item item)
    {
