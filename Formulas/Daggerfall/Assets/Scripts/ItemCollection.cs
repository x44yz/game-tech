...
public class ItemCollection
{
    ...
    {
        int locGroupIndex = Items.GetGroupIndex(ItemGroups.MiscItems, (int)MiscItems.Letter_of_credit);
        int amount = 0;
        foreach (Item item in items.Values)
        {
    ...
    public void AddItem(Item item, AddPosition position = AddPosition.Back, bool noStack = false)
    {
        if (item == null)
            return;
        ...
        Item stack = FindExistingStack(item);
    ...
    public void ReorderItem(Item item, AddPosition position)
    {
        if (!items.Contains(item.uid))
            return;
    ...
    public Item GetItem(ItemGroups itemGroup, int itemIndex, bool priorityToConjured)
    {
    ...
    public void CopyAll(ItemCollection source)
    {
        ...
        foreach (Item item in source.items.Values)
        {
            AddItem(item.Clone());
        }
    }
    ...
    public void Transfer(Item item, ItemCollection source, AddPosition position = AddPosition.DontCare)
    {
    ...
    // public Item[] ExportQuestItems(ulong questUID, Symbol itemSymbol)
    // {
    //     if (itemSymbol == null)
    //         return null;
    ...
    //             {
    //                 Item modItem = (Item)Activator.CreateInstance(itemClassType);
    //                 modItem.FromItemData(itemArray[i]);
    //                 AddItem(modItem, noStack: true);
    //                 continue;
    //             }
    //         }
    //         Item item = new Item(itemArray[i]);
    ...
    public List<Item> SearchItems(ItemGroups itemGroup, int templateIndex = -1)
    {
        List<Item> results = new List<Item>();
        foreach (Item item in items.Values)
        {
    ...
    Item FindExistingStack(Item item)
    {
        if (!item.IsStackable())
            return null;
        ...
        foreach (Item checkItem in items.Values)
        {
            if (checkItem != item && 
