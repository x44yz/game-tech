    ...
    public class dfMonster : Unit
    {
        ...
        */
        ...
        void InitMonster(int typeIndex)
        {
            ...
            int maxhp = data.hitPointsMinimum + dfUtils.GenerateRnd(data.hitPointsMaximum - data.hitPointsMinimum + 1);
            ...
            rndItemSeed = dfUtils.AdvanceRndSeed();
            aiSeed = dfUtils.AdvanceRndSeed();
            whoHit = 0;
            toHit = data.toHit;
        ...
        public void AttackPlayer(dfPlayer player)
        {
            MonsterAttackPlayer(player, toHit, minDamage, maxDamage);
        }

        void MonsterAttackPlayer(dfPlayer player, int hit, int minDam, int maxDam)
        {
            if (player._pHitPoints >> dfDEF.HPMANAOFFSET <= 0 || player._pInvincible || dfUtils.HasAnyOf(player._pSpellFlags, SpellFlag.Etherealize))
                return;
            // 判断距离
            // if (position.tile.WalkingDistance(player.position.tile) >= 2)
            //     return;

            int hper = dfUtils.GenerateRnd(100);
        #if _DEBUG
            if (DebugGodMode)
                hper = 1000;
        #endif
            ...
            // 针对
            if (dfUtils.HasAnyOf(player.pDamAcFlags, ItemSpecialEffectHf.ACAgainstDemons) && data.monsterClass == MonsterClass.Demon)
                ac += 40;
            if (dfUtils.HasAnyOf(player.pDamAcFlags, ItemSpecialEffectHf.ACAgainstUndead) && data.monsterClass == MonsterClass.Undead)
                ac += 20;
            ...
            if (hper >= hit)
            {
                dfTest.Inst.ShowDamageText(player, 0);
                return;
            }
            ...
            if ((player._pmode == PLR_MODE.PM_STAND || player._pmode == PLR_MODE.PM_ATTACK) && player._pBlockFlag) {
                blkper = dfUtils.GenerateRnd(100);
            }
            // blk 越大越容易闪避
            ...
            if (blkper < blk) 
            {
                Direction dir = dfUtils.GetDirection(player.position.tile, position.tile);
                player.StartPlrBlock(dir);
                if (/*player == MyPlayer &&*/ player.wReflections > 0) 
                {
                    int kdam = dfUtils.GenerateRnd(((maxDam - minDam) << dfDEF.HPMANAOFFSET) + 1) + (minDam << dfDEF.HPMANAOFFSET);
            ...
            int dam = (minDam << dfDEF.HPMANAOFFSET) + dfUtils.GenerateRnd(((maxDam - minDam) << dfDEF.HPMANAOFFSET) + 1);
            // 最小 64 点伤害？
            // _pIGetHit 减伤
            ...
            if (dfUtils.HasAnyOf(player._pIFlags, ItemSpecialEffect.Thorns) && mode != MonsterMode.Death) 
            {
                int mdam = (dfUtils.GenerateRnd(3) + 1) << dfDEF.HPMANAOFFSET;
        ...
        void CheckReflect(dfPlayer player, ref int dam)
        {
            player.wReflections--;
            // if (player.wReflections <= 0)
            //     NetSendCmdParam1(true, CMD_SETREFLECT, 0);
            // reflects 20-30% damage
            int mdam = dam * (dfUtils.GenerateRnd(10) + 20) / 100;
            ApplyMonsterDamage(mdam);
        ...
        public void M_StartKill(dfPlayer player, int dam)
        {
        ...
        public void M_StartHit(dfPlayer player, int dam)
        {
            // tag(player);
            // if (IsHardHit(monster, dam)) {
            //     enemy = player.getId();
