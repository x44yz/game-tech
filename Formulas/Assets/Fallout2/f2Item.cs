...
namespace f2
{
    ...
    */
    ...
    public class f2Item
    {
        ...
        public void RecreateItem(f2Player player, _item_indexes idx, int icreateinfo, int iseed, int ivalue)
        {
        ...
        void RecreateTownItem(f2Player player, _item_indexes idx, int icreateinfo, int iseed)
        {
        ...
        void RecreateSmithItem(f2Player player, int lvl, int iseed)
        {
            f2Utils.SetRndSeed(iseed);
        ...
        _item_indexes RndSmithItem(f2Player player, int lvl)
        {
            return RndVendorItem(SmithItemOk, true, player, 0, lvl);
        }

        bool SmithItemOk(f2Player player, int i)
        {
        ...
        _item_indexes RndVendorItem(Func<f2Player, int, bool> Ok, bool ConsiderDropRate, f2Player player, int minlvl, int maxlvl)
        {
            ...
            return ril[f2Utils.GenerateRnd(ril.Count)];
        }
        ...
        void GetItemAttrs(_item_indexes itemData, int lvl)
        {
            ...
            _iAC = baseItemData.iMinAC + f2Utils.GenerateRnd(baseItemData.iMaxAC - baseItemData.iMinAC + 1);
        ...
        void SetupAllUseful(int iseed, int lvl)
        {
            ...
            if (dfDEF.gbIsHellfire) {
                ...
                idx = f2Utils.PickRandomlyAmong<_item_indexes>(new List<_item_indexes>{ _item_indexes.IDI_MANA, _item_indexes.IDI_HEAL });

                if (lvl > 1 && f2Utils.FlipCoin(3))
                    idx = _item_indexes.IDI_PORTAL;
            }
        ...
        void SetupAllItems(f2Player player, _item_indexes idx, int iseed, int lvl, int uper, bool onlygood, bool recreate, bool pregen)
        {
            _iSeed = iseed;
            f2Utils.SetRndSeed(iseed);
            ...
            if (_iMiscId != item_misc_id.IMISC_UNIQUE) {
                int iblvl = -1;
                if (f2Utils.GenerateRnd(100) <= 10 || f2Utils.GenerateRnd(100) <= lvl) {
                    iblvl = lvl;
                }
        ...
        public int _iUid = 0;
        void GetUniqueItem(f2Player player, _unique_items uid)
        {
        ...
        void ItemRndDur()
        {
            if (_iDurability > 0 && _iDurability != DUR_INDESTRUCTIBLE)
                _iDurability = f2Utils.GenerateRnd(_iMaxDur / 2) + (_iMaxDur / 4) + 1;
        }

        public void GenerateNewSeed()
        {
            _iSeed = f2Utils.AdvanceRndSeed();
        }
        ...
        // item 加成
        void GetItemBonus(f2Player player, int minlvl, int maxlvl, bool onlygood, bool allowspells)
        {
            if (minlvl > 25)
                minlvl = 25;
        ...
        void GetItemPower(f2Player player, int minlvl, int maxlvl, AffixItemType flgs, bool onlygood)
        {
            int[] l = new int[256];
            goodorevil goe;

            bool allocatePrefix = f2Utils.FlipCoin(4);
            bool allocateSuffix = !f2Utils.FlipCoin(3);
            ...
            if (!onlygood && !f2Utils.FlipCoin(3))
                onlygood = true;
            if (allocatePrefix) {
                int nt = 0;
                for (int j = 0; dfData.ItemPrefixes[j].power.type != item_effect_type.IPL_INVALID; j++) {
                    ...
                    if (onlygood && !dfData.ItemPrefixes[j].PLOk)
                        continue;
                    if (f2Utils.HasAnyOf(flgs, AffixItemType.Staff) && dfData.ItemPrefixes[j].power.type == item_effect_type.IPL_CHARGES)
                        continue;
                    l[nt] = j;
                    nt++;
                ...
                if (nt != 0) {
                    preidx = l[f2Utils.GenerateRnd(nt)];
            ...
            if (allocateSuffix) {
                ...
                if (nl != 0) {
                    sufidx = l[f2Utils.GenerateRnd(nl)];
                    _iMagical = item_quality.ITEM_QUALITY_MAGIC;
        ...
        bool IsSuffixValidForItemType(int i, AffixItemType flgs)
        {
            ...
            return f2Utils.HasAnyOf(flgs, itemTypes);
        }
        ...
        void SaveItemAffix(f2Player player, PLStruct affix)
        {
        ...
        int CalculateToHitBonus(int level)
        {
            switch (level) {
            case -50:
                return -f2Utils.RndPL(6, 10);
            case -25:
                return -f2Utils.RndPL(1, 5);
            case 20:
                return f2Utils.RndPL(1, 5);
            case 36:
                return f2Utils.RndPL(6, 10);
            case 51:
                return f2Utils.RndPL(11, 15);
            case 66:
                return f2Utils.RndPL(16, 20);
            case 81:
                return f2Utils.RndPL(21, 30);
            case 96:
                return f2Utils.RndPL(31, 40);
            case 111:
                return f2Utils.RndPL(41, 50);
            case 126:
                return f2Utils.RndPL(51, 75);
            case 151:
                return f2Utils.RndPL(76, 100);
            default:
        ...
        int SaveItemPower(f2Player player, ItemPower power)
        {
            if (!dfDEF.gbIsHellfire) {
            ...
            int r = f2Utils.RndPL(power.param1, power.param2);
            ...
            case item_effect_type.IPL_TOHIT_DAMP:
                r = f2Utils.RndPL(power.param1, power.param2);
                _iPLDam += r;
        ...
        void GetStaffSpell(f2Player player, int lvl, bool onlygood)
        {
            if (!dfDEF.gbIsHellfire && f2Utils.FlipCoin(4)) {
            ...
            if (l == 0)
                l = 1;
            int rv = f2Utils.GenerateRnd(maxSpells) + 1;
            ...
            _iSpell = bs;
            _iCharges = minc + f2Utils.GenerateRnd(maxc);
            _iMaxCharges = _iCharges;
        ...
        void GetStaffPower(f2Player player, int lvl, int bs, bool onlygood)
        {
            int preidx = -1;
            if (f2Utils.FlipCoin(10) || onlygood) 
            {
                int nl = 0;
                int[] l = new int[256];
                for (int j = 0; dfData.ItemPrefixes[j].power.type != item_effect_type.IPL_INVALID; j++) 
                ...
                if (nl != 0) {
                    preidx = l[f2Utils.GenerateRnd(nl)];
                    _iMagical = item_quality.ITEM_QUALITY_MAGIC;
                    SaveItemAffix(player, dfData.ItemPrefixes[preidx]);
                    _iPrePower = dfData.ItemPrefixes[preidx].power.type;
                }
