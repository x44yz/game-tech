...
namespace f2
{
    public class f2Player : Unit
    {
        ...
        public f2Item[] InvBody = new f2Item[(int)inv_body_loc.NUM_INVLOC];
        ...
        public f2Item[] InvList = new f2Item[InventoryGridCells];
        public int _pNumInv;
        public f2Item[] SpdList = new f2Item[MaxBeltItems];
        void CreatePlrItems()
        {
            ...
            f2Item goldItem = InvList[_pNumInv];
        ...
        bool AutoPlaceItemInInventorySlot(int slotIndex, f2Item item, bool persistItem)
        {
        ...
        void MakeGoldStack(f2Item goldItem, int value)
        {
        ...
        */
        ...
        public void AttackMonster(f2Monster monster)
        {
            StartPlayerAttack();
        ...
        public bool PlrHitMonster(f2Monster monster, bool adjacentDamage = false)
        {
            // 击中概率，用于 miss 判定
            // hper = hit percent?
            int hper = 0;
            ...
            int hit = f2Utils.GenerateRnd(100);
            // 石化状态，必然命中
            ...
            if (hit >= hper) {
                ...
                f2Test.Inst.ShowMiss(monster);
                return false;
            }

            if (dfDEF.gbIsHellfire && f2Utils.HasAllOf(_pIFlags, ItemSpecialEffect.FireDamage | ItemSpecialEffect.LightningDamage)) 
            {
                int midam = _pIFMinDam + f2Utils.GenerateRnd(_pIFMaxDam - _pIFMinDam);
                // AddMissile(position.tile, position.temp, _pdir, MIS_SPECARROW, TARGET_MONSTERS, getId(), midam, 0);
            }

            int mind = _pIMinDam;
            int maxd = _pIMaxDam;
            int dam = f2Utils.GenerateRnd(maxd - mind + 1) + mind;
            ...
            {
                // 等级越高暴击概率越高
                if (f2Utils.GenerateRnd(100) < _pLevel) 
                {
                    dam *= 2;
                }
            }
            ...
            case MonsterClass.Demon:
                if (f2Utils.HasAnyOf(_pIFlags, ItemSpecialEffect.TripleDemonDamage)) {
                    dam *= 3;
                }
                break;
            }

            if (f2Utils.HasAnyOf(pDamAcFlags, ItemSpecialEffectHf.Devastation) && f2Utils.GenerateRnd(100) < 5) {
                dam *= 3;
            }
            ...
            if (f2Utils.HasAnyOf(pDamAcFlags, ItemSpecialEffectHf.Jesters)) 
            {
                int r = f2Utils.GenerateRnd(201);
                if (r >= 100)
                    r = 100 + (r - 100) * 5;
                dam = dam * r / 100;
            }
            ...
            // if (&player == MyPlayer) 
            {
                if (f2Utils.HasAnyOf(pDamAcFlags, ItemSpecialEffectHf.Peril)) 
                {
            ...
            int skdam = 0;
            if (f2Utils.HasAnyOf(_pIFlags, ItemSpecialEffect.RandomStealLife)) 
            {
                skdam = f2Utils.GenerateRnd(dam / 8);
                _pHitPoints += skdam;
            ...
            // 窃取魔法效果
            if (f2Utils.HasAnyOf(_pIFlags, ItemSpecialEffect.StealMana3 | ItemSpecialEffect.StealMana5) && f2Utils.HasNoneOf(_pIFlags, ItemSpecialEffect.NoMana)) 
            {
                if (f2Utils.HasAnyOf(_pIFlags, ItemSpecialEffect.StealMana3)) {
                    skdam = 3 * dam / 100;
                }
                if (f2Utils.HasAnyOf(_pIFlags, ItemSpecialEffect.StealMana5)) {
                    skdam = 5 * dam / 100;
                }
                _pMana += skdam;
                if (_pMana > _pMaxMana) 
            ...
            // 窃取 hp 
            if (f2Utils.HasAnyOf(_pIFlags, ItemSpecialEffect.StealLife3 | ItemSpecialEffect.StealLife5)) 
            {
                if (f2Utils.HasAnyOf(_pIFlags, ItemSpecialEffect.StealLife3)) {
                    skdam = 3 * dam / 100;
                }
                if (f2Utils.HasAnyOf(_pIFlags, ItemSpecialEffect.StealLife5)) {
                    skdam = 5 * dam / 100;
                }
                _pHitPoints += skdam;
                if (_pHitPoints > _pMaxHP) 
        ...
        void CalcPlrItemVals(bool loadgfx)
        {
            ...
            if (f2Utils.HasAnyOf(_pSpellFlags, SpellFlag.RageActive)) {
                sadd += 2 * _pLevel;
                dadd += _pLevel + _pLevel / 2;
                vadd += 2 * _pLevel;
            }
            if (f2Utils.HasAnyOf(_pSpellFlags, SpellFlag.RageCooldown)) {
                sadd -= 2 * _pLevel;
                dadd -= _pLevel + _pLevel / 2;
                vadd -= 2 * _pLevel;
            }
            ...
            if (_pClass == HeroClass.Rogue) {
                ...
                } else if (f2Utils.IsNoneOf(InvBody[((int)inv_body_loc.INVLOC_HAND_LEFT)]._itype, ItemType.Staff, ItemType.Bow) && f2Utils.IsNoneOf(InvBody[((int)inv_body_loc.INVLOC_HAND_RIGHT)]._itype, ItemType.Staff, ItemType.Bow)) {
            ...
            if (f2Utils.HasAnyOf(iflgs, ItemSpecialEffect.ZeroResistance)) {
            ...
            if (_pClass == HeroClass.Warrior) {
                ...
                vadd += vadd;
                vadd += (vadd / 4);
            } else if (f2Utils.IsAnyOf(_pClass, HeroClass.Rogue, HeroClass.Monk, HeroClass.Bard)) {
                vadd += vadd / 2;
            }
            ...
            if (f2Utils.IsAnyOf(_pClass, HeroClass.Rogue, HeroClass.Monk)) {
                madd += madd / 2;
            } else if (_pClass == HeroClass.Bard) {
                madd += (madd / 4) + (madd / 2);
            }
        ...
        item_equip_type GetItemLocation(f2Item item)
        {
            if (_pClass == HeroClass.Barbarian && item._iLoc == item_equip_type.ILOC_TWOHAND && f2Utils.IsAnyOf(item._itype, ItemType.Sword, ItemType.Mace))
        ...
        void NextPlrLevel()
        {
            ...
            if (f2Utils.HasNoneOf(_pIFlags, ItemSpecialEffect.NoMana)) {
                _pMana = _pMaxMana;
                _pManaBase = _pMaxManaBase;
            }
        ...
        */
        ...
        {
            f2Test.Inst.ShowUnitText(this, "BLOCK");
        ...
        public bool AttackPlayer(f2Player target)
        {
            StartPlayerAttack();
            return PlrHitPlr(target);
        }

        public bool PlrHitPlr(f2Player target)
        {
            var attacker = this;
            ...
            if (f2Utils.HasAnyOf(target._pSpellFlags, SpellFlag.Etherealize)) 
            {
            ...
            if (blk < blkper)
            {
                Direction dir = f2Utils.GetDirection(target.position.tile, attacker.position.tile);
            ...
            if (attacker._pClass == HeroClass.Warrior || attacker._pClass == HeroClass.Barbarian) {
                if (f2Utils.GenerateRnd(100) < attacker._pLevel) {
                    dam *= 2;
                }
            }
            int skdam = dam << dfDEF.HPMANAOFFSET;
            if (f2Utils.HasAnyOf(attacker._pIFlags, ItemSpecialEffect.RandomStealLife)) {
                int tac = f2Utils.GenerateRnd(skdam / 8);
        ...
        void DamageArmor()
        {
            ...
            bool targetHead = f2Utils.FlipCoin(3);
            if (!InvBody[(int)inv_body_loc.INVLOC_CHEST].isEmpty() && InvBody[(int)inv_body_loc.INVLOC_HEAD].isEmpty()) {
                targetHead = false;
            }
            if (InvBody[(int)inv_body_loc.INVLOC_CHEST].isEmpty() && !InvBody[(int)inv_body_loc.INVLOC_HEAD].isEmpty()) {
            ...
            if (pi._iDurability == f2Item.DUR_INDESTRUCTIBLE) {
                return;
            }
        ...
        {
            if (!InvBody[ii].isEmpty() && InvBody[ii]._iClass == item_class.ICLASS_WEAPON && f2Utils.HasAnyOf(InvBody[ii]._iDamAcFlags, ItemSpecialEffectHf.Decay)) {
        ...
        bool DamageWeapon(int damageFrequency)
        {
            ...
            if (!f2Utils.FlipCoin(damageFrequency)) {
                return false;
            }
            ...
            if (!InvBody[(int)inv_body_loc.INVLOC_HAND_LEFT].isEmpty() && InvBody[(int)inv_body_loc.INVLOC_HAND_LEFT]._iClass == item_class.ICLASS_WEAPON) {
                if (InvBody[(int)inv_body_loc.INVLOC_HAND_LEFT]._iDurability == f2Item.DUR_INDESTRUCTIBLE) {
                    return false;
                }
            ...
            if (!InvBody[(int)inv_body_loc.INVLOC_HAND_RIGHT].isEmpty() && InvBody[(int)inv_body_loc.INVLOC_HAND_RIGHT]._iClass == item_class.ICLASS_WEAPON) {
                if (InvBody[(int)inv_body_loc.INVLOC_HAND_RIGHT]._iDurability == f2Item.DUR_INDESTRUCTIBLE) {
                    return false;
                }
